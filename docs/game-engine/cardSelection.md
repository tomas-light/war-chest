...
> Статус: правила реализованы 14 сентября 2026 года для базового набора карт
> с уточнённой связью первого игрока и командной очереди.
...
2. Каждый ручной ход — выбор и подтверждение ровно одной доступной карты.
...
4. Проходы вперёд и назад повторяются, пока каждый не получит нужное
   количество карт. Единственная оставшаяся карта автоматически достаётся
   следующему игроку в очереди; подтверждать её не нужно.
...
| Формат        | Игроков | Карт у каждого | Проходов | Ручных выборов | Автоматических выдач |
| ------------- | ------- | -------------- | -------- | -------------- | -------------------- |
| Дуэльный 1×1  | 2       | 4              | 4        | 7              | 1                    |
| Командный 2×2 | 4       | 3              | 3        | 11             | 1                    |

В дуэли полная очередь: **1 → 2 → 2 → 1 → 1 → 2 → 2 → 1**.
После семи ручных выборов игрок 1 автоматически получает последнюю карту.
У обоих игроков по четыре карты.

В командной партии полная очередь:
**1 → 2 → 3 → 4 → 4 → 3 → 2 → 1 → 1 → 2 → 3 → 4**.
Игроки 1 и 3 — союзники, игроки 2 и 4 входят в противоположную команду.
После одиннадцати ручных выборов игрок 4 автоматически получает последнюю
карту. У каждого игрока по три карты, у каждой команды — по шесть.

Очереди и схема показывают порядок получения карт, включая последнюю
автоматическую выдачу, а не только ручные действия.

![Общая реверсивная очередь выбора для дуэли и команд](./images/cardSelectionOrder.svg)

Игрок 1 выбирается случайно из всех участников при переходе от подготовки
к выбору карт. В командной партии игрок 2 берётся из противоположной команды:
если там два участника, первым становится игрок с меньшим номером места.
Игрок 3 — союзник игрока 1, игрок 4 — оставшийся союзник игрока 2. Поэтому
команды в очереди чередуются независимо от цвета команды первого игрока.

Игрок 1 начинает бан или выбор и получает инициативу в начале партии. В
режиме Random ручного этапа нет: первый игрок определяется при переходе сразу
к столу и там же получает инициативу.

В будущем экран подготовки сможет задавать режим определения первого игрока:
случайный выбор либо явный выбор одного места из двух команд. Сейчас доступен
только случайный режим; настройка ещё не входит в данные подготовки.
...
Ручной выбор подтверждается отдельным действием: до подтверждения его
...
Production-каталог сейчас содержит 16 карт базовой игры. Составы карт
Nobility, Siege и Nightfall и их ассеты ещё не определены, поэтому этап не
добавляет выдуманные карты дополнений.

После подтверждения предпоследней доступной карты движок сразу выдаёт
последнюю следующему игроку в очереди и переводит партию к столу. Это правило
одинаково для Draft и Elimination Draft. Автоматическая выдача не требует
подключения получателя; право первого хода принадлежит первому игроку партии.
Нет отдельного подтверждения последней карты, кнопки «К игровому столу» или
экрана ожидания подтверждения итогов.

Итоговые раскладки в Figma сохранены как справочные снимки автоматической
раздачи, а не как самостоятельный шаг пользователя.
...
15 сентября правило уточнено: последний ручной выбор делается из двух карт,
оставшаяся выдаётся автоматически, и сразу открывается стол.
