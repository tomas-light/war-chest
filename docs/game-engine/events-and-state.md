...
1. Сервер передаёт текущее состояние, идентификатор игрока и команду в
   `decide`.
2. Движок возвращает `GameEventData[]`. В текущем сценарии в нём ноль или одно
   событие.
...
Значения `FeatureFlags` всегда примитивны: `boolean`, `number`, `string` или
`null`. Поэтому при переносе snapshot достаточно поверхностной копии объекта;
рекурсивное клонирование для feature flags не выполняется.
...
Команды распределены между двумя функциями:
...
| --- | --- | --- |
| `GameCreated` | `featureFlags`, `rulesVersion` | Создаёт состояние `waiting` и фиксирует флаги |
| `PlayerJoined` | `playerId`, `seat` | Добавляет игрока с пустой историей тестовых ходов |
...
| `GameFinished` | `finishedByPlayerId` | Переводит игру в `finished` и очищает текущий ход |
...
В `GameCreated` дополнительно хранится `rulesVersion`. Сейчас
...
завершает восстановление ошибкой. Функция не сортирует события, не проверяет
пропуски `sequence` и не валидирует остальные доменные переходы. Сервер обязан
загрузить упорядоченную историю.
