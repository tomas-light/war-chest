...
Payload игровых сообщений проверяется Zod-схемой из
`@war-chest/api-contracts`. `lobby:subscribe` не принимает payload и
подтверждает callback после входа socket в комнату лобби.
...
| `lobby:subscribe` | входит в общую комнату обновлений лобби                               |
...
`lobby:updated` содержит только `gameId`. Он отправляется подписчикам после
создания игры, каждого сохранённого изменения и удаления просроченной пустой
`waiting`-игры. Клиент использует сообщение как сигнал заново запросить
`GET /api/games`: добавление игры, смена состава, запуск и исчезновение игры из
лобби не требуют ручного обновления страницы.
...
`commandId` не раскрывает identity ранее сохранённой команды. Если пустая
`waiting`-игра удалена по таймауту, sockets её комнаты получают `game:error` с
кодом `game_not_found`.
