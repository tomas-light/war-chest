...
| `POST` | `/api/games/:gameId/settings`       | session     | изменяет настройки подготовки        |
...
`POST /api/games` принимает `commandId` и формат `duel`/`team`. Он создаёт
пустую игру без участников с базовым составом и случайным выбором карт:
создатель выбирает место отдельной командой после перехода на страницу игры и
до этого остаётся зрителем. Server сохраняет формат, начальные настройки и
snapshot runtime feature flags в `GameCreated`. Новая игра отвечает `201`, exact
duplicate — `200` с тем же `gameId` и актуальным безопасным view. Exact
duplicate возвращается без повторного чтения runtime feature flags.
...
`POST /api/games/:gameId/settings` принимает `commandId`, `expectedVersion`,
способ выбора карт и список дополнений. До запуска только создатель может
изменить эти параметры; формат партии остаётся неизменяемым. Server отклоняет
повторы дополнений и наборы, выключенные в сохранённом snapshot feature flags.
Успешная команда создаёт `GameSettingsUpdated`, обновляет проекцию `games` и
возвращает актуальный `GameResponse`, поэтому участники и зрители получают те
же настройки через обычный event tail.
