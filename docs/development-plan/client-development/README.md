...
игрока. При первом присоединении вкладка передаёт выбранную в UI свободную
позицию: связанные поля `team` и `seat`. Fake backend отклоняет частично
заполненную, уже занятую или неизвестную позицию. Для совместной партии нужно
войти через две разные provider-кнопки.
