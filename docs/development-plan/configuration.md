...
## Конфигурация приложений и базы

У каждого приложения и у пакета базы есть собственные файлы. Конфигурация
PostgreSQL находится рядом со схемой и миграциями, поэтому вся граница базы
переносится и настраивается как единое целое:
...
- искать файлы относительно корня конкретного приложения или пакета базы;
...
наполнении тестовыми данными и открытии Drizzle Studio. Для него действует тот
...
- каждое приложение и пакет базы читают собственный `env.yaml`;
