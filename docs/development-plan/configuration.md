...
Endpoint при запросе читает runtime-файл активного окружения, проверяет, что
верхний уровень является объектом, а все значения имеют тип `boolean`, и
возвращает полученный snapshot. Сервер не знает заранее список ключей и не
проверяет полноту их набора.
...
- сервер проверяет JSON-объект и boolean-тип каждого значения, но не знает
  список ключей и не проверяет полноту их набора;
