...
Текущий auth-only адаптер `apps/web` использует фасад напрямую для seeded
пользователей и auth sessions. В целевой архитектуре полного fake backend именно
`SharedWorker` владеет одним экземпляром фасада, благодаря чему вкладки одного
origin последовательно изменяют общую IndexedDB; перенос auth-вызовов в worker
не изменит API самого пакета.
