# Документация War Chest

Здесь собираем архитектурные решения, планы и описание механик проекта. Документация должна помогать понять не только то, как что-то устроено, но и зачем мы выбрали именно такой подход.

## Разделы

- [План разработки веб-игры](./development-plan/README.md) — путь от текущего каркаса монорепозитория до первого рабочего мультиплеерного сценария.
- [База данных](./database/README.md) — реализованная PostgreSQL-схема,
  конфигурация пакета, миграции и команды для локальной работы.
- [Fake database](./fake-database/README.md) — реализованная IndexedDB-схема,
  табличный слой, репозитории и начальные данные для разработки без сервера.
- [Авторизация](./auth/README.md) — фактическое поведение OAuth-провайдеров,
  сессий и обработки аватаров в `@war-chest/auth`.
- [Game engine](./game-engine/README.md) — согласованные режимы игры и выбор
  карт; реализованный технический сценарий, события, восстановление состояния
  и безопасные представления.
- [Web-клиент](./client/README.md) — фактические маршруты, runtime-поведение,
  сборка и тестовые контуры `apps/web`.
- [Визуальный стиль UI](./ui/README.md) — правила cyberpunk-steampunk
  оформления, палитра, компоненты, анимации и критерии проверки новых экранов.
- [Вёрстка по Figma](./ui/figma.md) — макеты War Chest, компоненты и токены,
  правила переноса в приложение и текущее покрытие состояний.
- [Server](./server/README.md) — фактический HTTP API, Socket.IO и раздача SPA
  из `apps/server`.

## Зависимости приложений и пакетов

В монорепозитории два приложения и шесть пакетов. Схема ниже разделяет уже
объявленные workspace-зависимости и связи, которые зафиксированы в плане
разработки, но пока не добавлены в `package.json`.

![Зависимости приложений и пакетов War Chest](./images/workspace-dependencies.svg)

Стрелка читается как «узел слева использует узел справа». Сплошные стрелки —
текущие зависимости из `package.json`, пунктирные — целевые зависимости из
плана разработки.

Связь клиента с сервером через HTTP и Socket.IO на схеме не показана: это
runtime-взаимодействие, а не импорт workspace-пакета. `fake-database` использует
доменные типы `database` через type-only imports, но остаётся отделён от
production-контура: клиент должен загружать его только в development-сборке
после выбора fake backend.

`api-contracts` содержит общие TypeScript-типы и Zod-схемы сообщений для
сервера, web-клиента и будущего fake backend. TypeScript описывает контракт во
время разработки, а Zod отбрасывает несовместимый JSON во время выполнения.

Технический пакет `config` не владеет настройками конкретной части системы. Он
только одинаково читает `env.yaml`, необязательный `env.local.yaml` и известные
переменные процесса для `auth` и `database`; типизированные схемы остаются в
пакетах-потребителях.

## Как работать с документацией

План разработки — живой документ. По мере обсуждения мы будем уточнять решения, отмечать завершённые этапы и переносить устойчивые договорённости в отдельную архитектурную документацию.
