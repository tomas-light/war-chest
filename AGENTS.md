...
- Верстай интерфейс по [макетам War Chest в Figma](https://www.figma.com/design/3dryG8iK34xmpJF7HLYeau/War-Chest?node-id=15-5).
  Следуй [правилам работы с макетами](./docs/ui/figma.md): используй конкретный
  экран, роль и состояние, переиспользуй компоненты и токены, проверяй desktop
  и mobile. Если нужного состояния ещё нет в Figma, сохраняй его поведение
  в приложении и оформляй через существующие компоненты и визуальный контракт.
...
## Конкатенация CSS-классов в `apps/web`

- Для объединения CSS-классов используй `clsx`. Не собирай `className` вручную
  через массивы с `filter(Boolean).join(' ')`, шаблонные строки, оператор `+`
  или другие способы конкатенации строк.

```tsx
// ❌ Не надо так: классы объединяются вручную
className={[classes.link, className].filter(Boolean).join(' ')}

// ✅ Правильно: обязательные и переданные классы перечислены через clsx
className={clsx(classes.link, className)}
```

- Условные классы передавай в объекте: ключом должен быть класс, а значением —
  условие его добавления.

```tsx
className={clsx(classes.link, className, {
  [classes.magicLink]: isMagic,
})}
```

## Растровые изображения с прозрачным фоном

- Если изображению или иконке нужен прозрачный фон, явно требуй от генератора
  настоящий прозрачный фон с alpha-каналом. Отдельно запрещай рисовать
  шахматную подложку, белый фон или другой фон, имитирующий прозрачность.
- Не считай шахматную подложку в preview доказательством прозрачности: её может
  показывать viewer, а может содержать само изображение. После экспорта проверь
  формат PNG, наличие alpha-канала и прозрачность пикселей вне силуэта.
- Не принимай opaque RGB-файл или PNG с нарисованной шахматкой как готовый
  transparent-asset. Перегенерируй его либо технически удали только внешний
  фон, затем повторно проверь изображение визуально и по значениям alpha.
- При resize, crop и compositing сохраняй alpha-канал и повторяй проверку уже
  для итогового файла в workspace, а не только для исходного результата
  генератора.
